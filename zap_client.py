...
class ZAPClient:
    ...
    async def get_spider_status(self, scan_id):
        ...
        if not scan_id:
            return 0
        client = await self._get_client()
        resp = await client.get(f"{self.api_url}/spider/view/status/", params={"scanId": scan_id})
        if resp.status_code != 200:
            return 0
        try:
            return int(resp.json().get("status", 0))
    ...
    async def get_spider_results(self, scan_id):
        ...
        client = await self._get_client()
        resp = await client.get(f"{self.api_url}/spider/view/results/", params={"scanId": scan_id})
        if resp.status_code != 200:
            return []
        try:
