...
| 🤖 **AI 취약점 분석** | LM Studio(로컬 LLM) / Google Gemini / Vertex AI를 통한 HTTP 트래픽 심층 분석 |
| 🎛️ **모듈식 스캔 제어** | ZAP Spider · FFuF · Deep Recon · AI 분석 단계별 개별 활성화/비활성화 |
| ⚡ **AI Only 모드** | 기존 정찰 결과 재사용, AI 분석만 단독 재실행 |
| 🎯 **수동 AI 타겟 지정** | 엔드포인트 맵에서 분석할 항목만 선택해 AI 전송 |
...
| 📂 **내장 워드리스트** | `wordlist/` 디렉토리에 번들된 FFuF용 사전 파일 지원 |
...
│
├── wordlist/                # 내장 FFuF 워드리스트 모음
│   ├── wordlist_last.txt                    # 기본 워드리스트 (자동 선택)
...
└── results/                 # 스캔 결과 저장 디렉토리 (자동 생성)
    ...
    │       ├── manual_targets.json      # 수동 지정 AI 분석 타겟 (선택적)
...
| `/api/zap/clear` | POST | ZAP 히스토리 초기화 + 현재 세션 recon_map 초기화 |
| `/api/wordlists` | GET | 내장 워드리스트 파일 목록 반환 |
| `/api/session/{id}/clear_log` | POST | 세션 스캔 로그 초기화 |
| `/api/session/{id}/manual_targets` | POST | 수동 AI 분석 타겟 저장 |
...
#### 스캔 요청 파라미터 (`POST /api/scan`)

| 파라미터 | 기본값 | 설명 |
|---------|--------|------|
| `target_url` | — | 스캔 대상 URL |
| `enable_zap_spider` | `true` | ZAP Spider 실행 여부 |
| `enable_ffuf` | `true` | FFuF 퍼징 실행 여부 |
| `enable_deep_recon` | `true` | Deep Recon (재귀 탐색) 실행 여부 |
| `enable_ai_analysis` | `true` | AI 취약점 분석 실행 여부 |
| `ffuf_options` | `-t 50 -mc 200,204,301,302,307,401,403,500 -ac -ic` | FFuF 추가 옵션 |
| `ffuf_wordlist` | `""` (내장 기본값 사용) | 워드리스트 파일명 |

> ZAP · FFuF를 모두 끄고 AI 분석만 켜면 **AI Only 모드**로 동작 — 기존 `recon_map.json`을 재사용합니다.
...
- **`run_recon_agent()`**: ZAP Spider → FFuF 퍼징 → Deep Recon (재귀 크롤링) → 최종 전수 동기화 순서로 엔드포인트를 수집. `enable_zap`, `enable_ffuf`, `ffuf_options`, `ffuf_wordlist` 파라미터로 단계별 제어 가능
- **`run_analysis_agent()`**: 수집된 HTTP 요청을 AI에 전송, 배치 처리로 취약점 도출, `ai_findings.json`에 누적 저장. `manual_targets.json` 존재 시 해당 항목만 분석
...
- **`run_ffuf()`**: FFuF 디렉토리 퍼징 실행, 결과를 비동기 스트림으로 반환. `ffuf_options`·`ffuf_wordlist` 파라미터로 실행 옵션 및 워드리스트 커스터마이징 가능
...
| `settings.js` | `saveSettings`, `loadSettings`, `toggleProxy`, `toggleAiSettings`, `fetchGeminiModels` | AI 설정 · 프록시 저장/불러오기, Gemini 모델 목록 자동 조회 |
...
| `scan.js` | `startScan`, `stopScan`, `pollZapHistory`, `analyzeSelectedEndpoints` | 스캔 실행, 단계별 토글, 워드리스트 선택, 수동 AI 타겟 지정 |
...
| 엔진 | 설정 항목 | 기본 모델 | 특징 |
|------|----------|----------|------|
| **LM Studio** (기본) | API URL + 모델명 | `qwen/qwen3.5-9b` | 로컬 실행, 인터넷 불필요 |
| **Google Gemini** | API Key + 모델 선택 | `gemini-2.0-flash` | 대용량 컨텍스트 (최대 500KB 배치), 모델 목록 자동 조회 |
| **Vertex AI** | Bearer Token | `gemini-2.0-flash` | Google Cloud 기반 |

> 엔진 전환 시 API URL과 모델명이 자동으로 변경됩니다.
...
프로젝트 생성
    ↓
[선택] ZAP Spider → 엔드포인트 수집
    ↓
[선택] FFuF 퍼징 → 숨겨진 경로 탐색
    ↓
[선택] Deep Recon → 신규 경로 재귀 크롤링
    ↓
ZAP 히스토리 전수 동기화
    ↓
[선택] AI 분석 (배치 처리) → 취약점 도출
         └─ 수동 타겟 지정 시: 선택 항목만 분석
    ↓
ai_findings.json 누적 저장 + 카드 렌더링
```

> **AI Only 모드**: ZAP Spider + FFuF를 모두 비활성화하면 기존 정찰 데이터를 그대로 유지하고 AI 분석만 재실행합니다.

---

## 📂 내장 워드리스트

`wordlist/` 디렉토리에 FFuF용 사전 파일이 번들되어 있습니다. UI의 워드리스트 선택 드롭다운에서 선택하거나 기본값(`wordlist_last.txt`)이 자동으로 사용됩니다.

| 파일 | 설명 |
|------|------|
| `wordlist_last.txt` | 기본 워드리스트 (자동 선택) |
| `directory-list-2.3-small.txt` | DirBuster 소형 목록 |
| `directory-list-2.3-medium.txt` | DirBuster 중형 목록 |
| `directory-list-lowercase-2.3-small.txt` | 소문자 전용 소형 목록 |
| `directory-list-lowercase-2.3-medium.txt` | 소문자 전용 중형 목록 |
| `directory-list-1.0.txt` | DirBuster 1.0 목록 |
| `directories.jbrofuzz` | JBroFuzz 디렉토리 목록 |
| `apache-user-enum-1.0.txt` | Apache 사용자 열거 목록 |
| `apache-user-enum-2.0.txt` | Apache 사용자 열거 목록 v2 |
